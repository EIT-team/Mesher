...
/*	unit = int(options["pixel_scale_mm"]);
	fa = options["facet_angle_deg"];
	fd = options["facet_distance_mm"];
	cre = options["cell_radius_edge_ratio"];
	cs = options["cell_fine_size_mm"];
	ccs = options["cell_coarse_size_mm"];
	e_R = options["electrode_radius_mm"];
